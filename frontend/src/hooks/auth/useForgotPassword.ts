...
import { useRequestResetCodeMutation, useVerifyResetCodeMutation, useResetPasswordMutation } from '../../service/auth/mutations';
...
export const useForgotPassword = () => {
    ...
    const handleVerifyCode = () => {
        ...
        verifyMutation.mutate({ email, code }, {
            onSuccess: () => {
                ...
                setSuccessMsg(t("auth.codeVerified") || "Code verified. You can now set a new password.");
            },
            onError: (err: any) => {
                setErrorMsg(err?.response?.data?.message || err.message || "Invalid or expired code");
            }
        });
    };
    ...
    const handleResetPassword = () => {
        ...
        if (!newPassword.trim() || newPassword.length < 6) {
            setErrorMsg(t("auth.passwordMinLength") || "Password must be at least 6 characters");
            return;
        }

        resetMutation.mutate({ email, newPassword }, {
            onSuccess: () => {
                setSuccessMsg(t("auth.passwordResetSuccess") || "Password successfully reset!");
                setTimeout(() => {
