    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat) => (
          <div
            key={stat.label}
            className="bg-white/80 backdrop-blur-xl p-6 rounded-3xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] hover:-translate-y-2 transition-all duration-500 group relative overflow-hidden"
          >
            {/* Decorative glow */}
            <div
            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
              <div
                className={`w-14 h-14 bg-gradient-to-br ${stat.color} rounded-2xl flex items-center justify-center shadow-lg ${stat.shadow} group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        {/* New Users Per Month — Area Chart */}
        <div className="lg:col-span-3 bg-white/80 backdrop-blur-xl rounded-3xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
          ...
          <div className="p-6 h-[280px]">
            {analyticsLoading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart
                  data={(analytics?.newUsersPerMonth || []).map((d) => ({
                ...
                >
                  ...
                  <Tooltip
                    ...
                    formatter={(val: number) => [`${val} users`, "New Users"]}
                  />
                  <Area
                    type="monotone"
                    dataKey="users"
        ...
        {/* Most Active Cities — Horizontal Bar Chart */}
          ...
          <div className="p-6 h-[280px]">
            {analyticsLoading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  layout="vertical"
                  data={(analytics?.mostActiveCities || [])
                    .slice(0, 7)
                    .map((d) => ({
                      city: d.city || "Unknown",
                ...
                >
                  ...
                  <Tooltip
                    ...
                    formatter={(val) => [`${val ?? 0} listings`, "Active Cars"]}
                  />
                  <Bar dataKey="listings" radius={[0, 6, 6, 0]} maxBarSize={18}>
                    {(analytics?.mostActiveCities || [])
                      .slice(0, 7)
                      .map((_d, index) => {
                        const colors = [
                          "#3b82f6",
